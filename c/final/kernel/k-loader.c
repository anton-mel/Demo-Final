...
int program_load(proc* p, int programnumber,
    ...
    // highest address any segment (text/data/bss) reaches
    elf_program* ph = (elf_program*) ((const uint8_t*) eh + eh->e_phoff);
    uintptr_t max_end_mem = 0;
    for (int i = 0; i < eh->e_phnum; ++i) {
        if (ph[i].p_type == ELF_PTYPE_LOAD) {
            ...
            uintptr_t end_mem = ph[i].p_va + ph[i].p_memsz;
    ...
    // the break starts there too, i.e. a zero-sized heap.
    p->program_break = ROUNDUP(max_end_mem, PAGESIZE);
