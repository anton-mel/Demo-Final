...
int exists_in_between(ptr_with_size *ptrs, void * ptr, long size, int start, int len){
    
    for(int i = start; i < len ; i++){
	if(ptrs[i].ptr == ptr && ptrs[i].size <= size)
	    return 1;
	if(ptrs[i].size != size){
	    app_printf(0, "size (%ld, %ld) [%d, %d] ", size, ptrs[i].size, start, len - 1);
	    return 0;
	}
    }
...
void process_main(void) {
	...
	if(i != 0 && ptr[i].size != ptr[i - 1].size){
	    start = i;
	}
	int r = exists_in_between(ptr, h.ptr_array[i], h.size_array[i], start, ptr_size);
	assert(r);
    }
