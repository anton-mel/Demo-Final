...
//    be a multiple of PAGESIZE). Returns 0 on success, or -1 without
//    changing the break at all if the new break would fall below the
//    heap's start (`p->original_break`) or at/past the top of the stack
//    (fixed at MEMSIZE_VIRTUAL - PAGESIZE for this assignment).
//
//    Growing the heap does not map any pages here: that's the
//    optimistic/lazy allocation this assignment asks for -- `sbrk` just
//    moves the break, and the first time the process actually touches a
//    new page it takes a page fault, handled in `exception`'s
//    INT_PAGEFAULT case below. Shrinking the heap, by contrast, unmaps
//    and frees affected pages immediately, since there's no reason to
//    defer that. A page in the shrunk range may never have been faulted
//    in at all (if the process grew then shrank without touching it),
//    so only ever act on a page `virtual_memory_lookup` confirms is
//    actually mapped (`vam.pn >= 0`) -- skipping that check is a classic
//    way to end up freeing (or worse, silently corrupting the metadata
//    of) a page number that was never really allocated.
...
int sbrk(proc * p, intptr_t difference) {
    ...
    if (difference < 0) {
        uintptr_t old_top = ROUNDUP(old_break, PAGESIZE);
        for (uintptr_t addr = ROUNDUP(new_break, PAGESIZE); addr < old_top; addr += PAGESIZE) {
...
void exception(x86_64_registers* reg) {
    ...
    switch (reg->reg_intno) {
        ...
        case INT_SYS_BRK:
            {
                // brk(addr): set the break to an absolute address.
                uintptr_t addr = current->p_registers.reg_rdi;
                intptr_t difference = (intptr_t) (addr - current->program_break);
                current->p_registers.reg_rax = (sbrk(current, difference) == 0) ? 0 : (uint64_t) -1;
                break;
            }

        case INT_SYS_SBRK:
            {
                // sbrk(increment): move the break by a relative amount,
                // returning the *previous* break on success.
                intptr_t difference = (intptr_t) current->p_registers.reg_rdi;
                uintptr_t old_break = current->program_break;
                current->p_registers.reg_rax = (sbrk(current, difference) == 0) ? old_break : (uint64_t) -1;
                break;
            }
        ...
        case INT_PAGEFAULT:
            {
                ...
                uintptr_t addr = rcr2();
                int missing = !(reg->reg_err & PFERR_PRESENT);
                ...
                if ((reg->reg_err & PFERR_USER) && missing && in_heap) {
                    ...
                    if (pa != NULL && virtual_memory_map(current->p_pagetable, page_addr,
                                (uintptr_t) pa, PAGESIZE, PTE_P | PTE_W | PTE_U) >= 0) {
                        // current->p_state is already P_RUNNABLE; falls
                        // through to `run`/`schedule` below.
                    } else {
                ...
                const char* operation = reg->reg_err & PFERR_WRITE
                    ? "write" : "read";
                const char* problem = missing
                    ? "missing page" : "protection problem";
