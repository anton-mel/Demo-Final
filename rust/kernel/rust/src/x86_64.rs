// x86_64.rs (ported from shared/x86-64.h)
//
...
pub const fn pte_flags(pageentry: X86_64PageentryT) -> u64 {
    pageentry & 0xFFF
}
...
// Pseudo-descriptors used for LGDT and LIDT instructions
//
// NB: Rust's `repr(packed(N))` caps a field's alignment at N -- it does
// NOT force byte-tight packing the way C's `__attribute__((packed))`
// does. For a struct the hardware reads at fixed byte offsets (like this
// one and the TSS below), what we actually want is plain `repr(packed)`
// (equivalent to `packed(1)`), which removes all inter-field padding.
#[repr(C, packed)]
#[derive(Clone, Copy)]
